...
<div class="space-y-8" x-data="{ addLeadOpen: false, importLeadsOpen: false, activeView: localStorage.getItem('leadsView') || 'table' }" x-init="$watch('activeView', v => localStorage.setItem('leadsView', v))">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        ...
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('leads.quality-audit') }}" class="inline-flex items-center space-x-2 px-4 py-3 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 hover:bg-amber-100 border border-amber-200 dark:border-amber-800 font-bold text-sm rounded-xl shadow-sm transition-all">
    ...
    @php
        $statsUser = Auth::user();
        $isExec = ($statsUser->role === 'sales_executive' || $statsUser->isSalesExecutive());
        $isAdminOrCare = in_array($statsUser->role, ['super_admin', 'company_admin', 'hr', 'customer_care', 'telemarketer', 'sales_manager']);
        $baseQuery = \App\Models\Lead::query();
        if ($isExec) {
            $baseQuery->where('assigned_to', $statsUser->id);
        } elseif (!in_array($statsUser->role, ['super_admin', 'company_admin', 'hr', 'customer_care', 'telemarketer'])) {
        ...
        $flaggedLeadsCount = (clone $baseQuery)->where('is_flagged_fake', true)->count();
        $reachabilityScore = $totalLeadsCount > 0 ? round((($totalLeadsCount - $flaggedLeadsCount) / $totalLeadsCount) * 100, 1) : 100;

        // For management: calculate per-rep audit summary
        $repAuditList = [];
        if ($isAdminOrCare) {
            $reps = \App\Models\User::whereIn('role', ['sales_executive', 'telemarketer', 'sales_manager'])->orderBy('name')->get();
            foreach ($reps as $rep) {
                $repTotal = \App\Models\Lead::where('assigned_to', $rep->id)->count();
                if ($repTotal > 0) {
                    $repFlagged = \App\Models\Lead::where('assigned_to', $rep->id)->where('is_flagged_fake', true)->count();
                    $repFailPct = round(($repFlagged / $repTotal) * 100, 1);
                    $repAuditList[] = [
                        'rep' => $rep,
                        'total' => $repTotal,
                        'flagged' => $repFlagged,
                        'reachable' => $repTotal - $repFlagged,
                        'fail_pct' => $repFailPct,
                        'is_high_risk' => ($repFailPct >= 30 && $repTotal >= 3),
                    ];
                }
            }
            // Sort highest failure percentage first
            usort($repAuditList, fn($a, $b) => $b['fail_pct'] <=> $a['fail_pct']);
        }
    @endphp
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-150 dark:border-slate-700 shadow-sm flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-wider block">Closed Deals</span>
                <span class="text-2xl font-black text-emerald-600 mt-1 block">{{ number_format($closedWonCount) }}</span>
                <span class="text-[11px] text-emerald-600 font-semibold">{{ $convRate }}% conversion</span>
            </div>
        ...
        <!-- Reachability & Lead Integrity KPI -->
        <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border {{ $flaggedLeadsCount > 0 ? 'border-rose-200 bg-rose-50/20' : 'border-gray-150' }} dark:border-slate-700 shadow-sm flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold {{ $flaggedLeadsCount > 0 ? 'text-rose-600' : 'text-gray-400' }} uppercase tracking-wider block">
                    Data Reachability
                </span>
                <span class="text-2xl font-black {{ $flaggedLeadsCount > 0 ? 'text-rose-600' : 'text-emerald-600' }} mt-1 block">
                    {{ $reachabilityScore }}%
                </span>
                <span class="text-[11px] font-semibold {{ $flaggedLeadsCount > 0 ? 'text-rose-600' : 'text-emerald-600' }}">
                    {{ $flaggedLeadsCount }} flagged invalid/dead
                </span>
            </div>
            <div class="w-11 h-11 rounded-xl {{ $flaggedLeadsCount > 0 ? 'bg-rose-100 text-rose-600' : 'bg-emerald-50 text-emerald-600' }} flex items-center justify-center font-bold text-lg">
                {{ $flaggedLeadsCount > 0 ? '⚠️' : '🛡️' }}
            </div>
        </div>
    </div>

    @if($isExec && $flaggedLeadsCount > 0)
    <!-- Executive Warning Banner -->
    <div class="bg-amber-50 border-2 border-amber-300 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-start space-x-3.5">
            <div class="w-10 h-10 rounded-xl bg-amber-500 text-white flex items-center justify-center font-black text-xl flex-shrink-0">
                ⚠️
            </div>
            <div>
                <h3 class="text-sm font-extrabold text-amber-900">
                    Action Required: You have {{ $flaggedLeadsCount }} lead(s) flagged with unreachable or dead phone numbers
                </h3>
                <p class="text-xs text-amber-800 mt-1 leading-relaxed">
                    Under Buckcrest Havens CRM verification policy, prospects that fail 3 consecutive calls or have invalid numbers are automatically disqualified from your sales performance quota. You have <strong>48 hours</strong> to obtain active alternate phone numbers from these prospects and update them.
                </p>
            </div>
        </div>
        <div class="flex-shrink-0">
            <a href="{{ route('leads.index', ['flagged' => '1']) }}" class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl shadow-md transition-all whitespace-nowrap">
                Review My Flagged Leads ({{ $flaggedLeadsCount }})
            </a>
        </div>
    </div>
    @endif

    @if($isAdminOrCare && !empty($repAuditList))
    <!-- Management Lead Reachability & Quality Audit Panel -->
    <div x-data="{ openAudit: false }" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="p-4 bg-gradient-to-r from-slate-900 via-slate-800 to-dark-900 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 cursor-pointer" @click="openAudit = !openAudit">
            <div class="flex items-center space-x-3">
                <span class="text-2xl">🛡️</span>
                <div>
                    <div class="flex items-center space-x-2">
                        <h3 class="text-sm font-black tracking-tight text-white uppercase">Lead Quality &amp; Fraud Prevention Audit</h3>
                        @php
                            $highRiskCount = collect($repAuditList)->where('is_high_risk', true)->count();
                        @endphp
                        @if($highRiskCount > 0)
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-500 text-white animate-pulse">
                            🚨 {{ $highRiskCount }} High-Risk Reps Flagged
                        </span>
                        @else
                    ...
                    <p class="text-xs text-slate-300 mt-0.5">
                        Automated 3-strike phone reachability monitor. Executives submitting dead or fake phone lines are highlighted below.
                    </p>
                </div>
            </div>
            <button type="button" class="text-xs font-bold text-slate-300 hover:text-white bg-slate-700/60 px-3 py-1.5 rounded-xl border border-slate-600 flex items-center space-x-1">
                <span x-text="openAudit ? 'Hide Audit Table ▲' : 'View Officer Audit Breakdown ▼'"></span>
            </button>
        </div>

        <div x-show="openAudit" x-cloak class="p-5 border-t border-gray-100 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs mb-2">
                <div class="p-3 bg-gray-50 rounded-xl border border-gray-150">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Total Audited Leads</span>
                    <strong class="text-base text-dark-900">{{ number_format($totalLeadsCount) }}</strong>
                </div>
                <div class="p-3 bg-rose-50 rounded-xl border border-rose-200">
                    <span class="text-[10px] font-bold text-rose-500 uppercase tracking-wider block">Flagged Dead / Fake Numbers</span>
                    <strong class="text-base text-rose-700">{{ number_format($flaggedLeadsCount) }} leads disqualified</strong>
                </div>
                <div class="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                    <span class="text-[10px] font-bold text-emerald-600 uppercase tracking-wider block">Overall Pipeline Reachability</span>
                    <strong class="text-base text-emerald-700">{{ $reachabilityScore }}% connected</strong>
                </div>
            </div>

            <div class="overflow-x-auto rounded-xl border border-gray-200">
                <table class="w-full text-left text-xs">
                    <thead class="bg-gray-50 border-b border-gray-200 text-gray-500 font-bold uppercase text-[10px]">
                        <tr>
                            <th class="px-4 py-3">Sales Officer / Rep</th>
                            <th class="px-4 py-3 text-center">Total Assigned Leads</th>
                            <th class="px-4 py-3 text-center">Reachable Leads</th>
                            <th class="px-4 py-3 text-center">Flagged Dead / Fake</th>
                            <th class="px-4 py-3 text-center">Failure Rate</th>
                            <th class="px-4 py-3 text-center">Audit Status</th>
                            <th class="px-4 py-3 text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($repAuditList as $item)
                        <tr class="hover:bg-gray-50 {{ $item['is_high_risk'] ? 'bg-rose-50/40' : '' }}">
                            <td class="px-4 py-3">
                                <div class="font-bold text-dark-900">{{ $item['rep']->name }}</div>
                                <div class="text-[10px] text-gray-400">{{ $item['rep']->email }}</div>
                            </td>
                            <td class="px-4 py-3 text-center font-bold text-dark-800">
                                {{ $item['total'] }}
                            </td>
                            <td class="px-4 py-3 text-center font-bold text-emerald-600">
                                {{ $item['reachable'] }}
                            </td>
                            <td class="px-4 py-3 text-center font-extrabold {{ $item['flagged'] > 0 ? 'text-rose-600' : 'text-gray-400' }}">
                                {{ $item['flagged'] }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="font-bold {{ $item['fail_pct'] >= 30 ? 'text-rose-600' : ($item['fail_pct'] > 0 ? 'text-amber-600' : 'text-emerald-600') }}">
                                    {{ $item['fail_pct'] }}%
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if($item['is_high_risk'])
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black bg-rose-600 text-white">
                                    🚨 High Inactive Risk (>30%)
                                </span>
                                @elseif($item['flagged'] > 0)
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('leads.index', ['assigned_to' => $item['rep']->id, 'flagged' => '1']) }}" class="text-xs font-bold text-rose-600 hover:text-rose-800 hover:underline">
                                    View Fake Leads &rarr;
                                </a>
                            </td>
                        </tr>
                        @endforeach
            ...
            <p class="text-[11px] text-gray-400 italic">
                * Note: Reps with &gt;= 30% unreachable leads (e.g. 10 or 16 unreachable out of 20) are marked High Risk. Customer Care and HR can follow up with reps to verify prospect authenticity.
            </p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4">
        <form action="{{ route('leads.index') }}" method="GET" class="w-full flex flex-col md:flex-row gap-4 items-center">
            ...
            <!-- Reachability / Verification Filter -->
            <div class="w-full md:w-48">
                <select name="flagged" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none text-sm text-gray-700 bg-white">
                    <option value="">All Reachability</option>
                    <option value="0" {{ request('flagged') === '0' ? 'selected' : '' }}>✅ Verified / Reachable</option>
                    <option value="1" {{ request('flagged') === '1' ? 'selected' : '' }}>⚠️ Suspected Dead / Fake</option>
                </select>
            </div>
            ...
            <div class="w-full md:w-48">
                <select name="assigned_to" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none text-sm text-gray-700 bg-white">
                    <option value="">All Officers / Staff</option>
                    @foreach($officers as $officer)
                    <option value="{{ $officer->id }}" {{ request('assigned_to') == $officer->id ? 'selected' : '' }}>{{ $officer->name }} ({{ ucwords(str_replace('_', ' ', $officer->role)) }})</option>
                    @endforeach
                </select>
            </div>
            @endif
            ...
            <div class="kanban-column flex-shrink-0 w-72 flex flex-col rounded-2xl overflow-hidden"
                ...
                <div class="kanban-drop-zone flex-1 p-3 space-y-3 min-h-[120px] transition-colors duration-200"
                    ...
                    <div class="kanban-card bg-white rounded-xl p-4 shadow-sm border border-gray-100
                        ...
                        <div class="mt-1.5 px-2 py-0.5 bg-rose-50 border border-rose-200 rounded-lg flex items-center justify-between text-[10px] font-extrabold text-rose-700">
                            <span>⚠️ Dead / Invalid</span>
                            <span>{{ $lead->unreachable_count }}x Failed</span>
                        </div>
                        @elseif($lead->unreachable_count > 0)
                        <div class="mt-1 px-1.5 py-0.5 bg-amber-50 rounded text-[10px] text-amber-700 font-semibold">
                            📵 {{ $lead->unreachable_count }} failed call(s)
                        </div>
                        @endif
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-2">
                                ...
                                @if($lead->is_flagged_fake)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-100 text-rose-700 border border-rose-300" title="{{ $lead->flagged_reason }}">
                                    ⚠️ Fake / Dead Line ({{ $lead->unreachable_count }} tries)
                                </span>
                                @elseif($lead->unreachable_count > 0)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                    📵 {{ $lead->unreachable_count }}x unreachable
                                </span>
                                @endif
                            </div>
                            <div class="text-xs text-gray-500 space-y-0.5 mt-1">
                                <div>
                                    Phone: 
                                    <a href="tel:{{ $lead->phone_number }}" class="font-medium {{ $lead->is_flagged_fake ? 'text-rose-600 line-through' : 'text-gray-700 hover:text-brand-600' }}">
                                        {{ $lead->phone_number }}
                                    </a>
                                </div>
    ...
    <!-- Capture Lead Modal -->
    <div x-cloak x-show="addLeadOpen" class="fixed inset-0 z-50 overflow-y-auto bg-dark-900/60 backdrop-blur-sm transition-opacity p-4 sm:p-6 flex items-start justify-center">
        <div class="bg-white rounded-3xl max-w-2xl w-full shadow-2xl overflow-hidden my-4 sm:my-8 max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-4rem)] flex flex-col" @click.away="addLeadOpen = false">
            ...
            <div class="flex justify-between items-center px-6 py-5 md:px-8 border-b border-gray-100 shrink-0 bg-white">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Register a new client inquiry or sales prospect</p>
                </div>
                <button @click="addLeadOpen = false" class="text-gray-400 hover:text-gray-600 p-2 rounded-xl hover:bg-gray-100 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('leads.store') }}" method="POST" class="flex flex-col flex-1 overflow-hidden">
                @csrf
    ...
    <div x-cloak x-show="importLeadsOpen" class="fixed inset-0 z-50 overflow-y-auto bg-dark-900/60 backdrop-blur-sm transition-opacity p-4 sm:p-6 flex items-start justify-center">
        <div class="bg-white rounded-3xl max-w-lg w-full shadow-2xl p-6 md:p-8 space-y-6 my-4 sm:my-8" @click.away="importLeadsOpen = false">
