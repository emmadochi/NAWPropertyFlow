...
<head>
  ...
  <title>{{ !empty($isRescheduled) ? 'Site Inspection Rescheduled' : 'Site Inspection Scheduled' }}</title>
</head>
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 35px 30px; text-align: center; border-bottom: 4px solid #FEA500;">
              ...
              <p style="margin:5px 0 0; font-size:12px; color:#94a3b8; font-weight:600; text-transform:uppercase; letter-spacing:1px;">
                {{ $isRescheduled ? 'VIP Site Inspection (Updated Schedule)' : 'VIP Site Inspection Invitation' }}
              </p>
            </td>
          </tr>
            ...
            <td style="padding: 40px 35px;">
              ...
              @if($isRescheduled)
                <p style="margin:0 0 20px; font-size:14px; line-height:1.7; color:#475569;">
                  Please note that your upcoming site inspection for <strong>{{ $inspection->property->name }}</strong> has been shifted to the new confirmed date and time below:
                </p>
              @else
                <p style="margin:0 0 20px; font-size:14px; line-height:1.7; color:#475569;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f8fafc; border:1px solid #e2e8f0; border-radius:14px; margin: 25px 0; overflow:hidden;">
                <tr>
                  <td colspan="2" style="background-color:#0f172a; padding:12px 18px; font-size:11px; font-weight:800; color:#FEA500; text-transform:uppercase; letter-spacing:0.5px;">
                    {{ $isRescheduled ? 'Updated Tour Schedule' : 'Tour Schedule & Location' }}
                  </td>
                </tr>
                <tr>
                ...
                @if($isRescheduled && !empty($originalDate))
                <tr>
                  <td style="padding:12px 18px; border-bottom:1px solid #e2e8f0; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Previous Date</td>
                  <td style="padding:12px 18px; border-bottom:1px solid #e2e8f0; font-size:12px; color:#94a3b8; text-decoration:line-through; text-align:right;">
                    {{ \Carbon\Carbon::parse($originalDate)->format('l, d M Y at h:i A') }}
                  </td>
                </tr>
                @endif

                <tr>
                  <td style="padding:12px 18px; border-bottom:1px solid #e2e8f0; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">{{ $isRescheduled ? 'New Confirmed Time' : 'Date & Time' }}</td>
              ...
              <p style="margin:0 0 20px; font-size:14px; line-height:1.7; color:#475569;">
                If you have any questions or need further adjustments, you can contact your assigned host directly or update preferences in your Client Portal.
              </p>
