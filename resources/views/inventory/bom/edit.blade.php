...
@section('title', 'Edit BOM Benchmark')
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-semibold text-brand-600 dark:text-brand-400">Edit Benchmark</span>
            </div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-1">Edit Consumption Benchmark</h1>
        </div>
    ...
    <form method="POST" action="{{ route('inventory.bom.update', $bom) }}" class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-6 space-y-6 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="activity_name" value="{{ old('activity_name', $bom->activity_name) }}" required
            ...
            <div class="space-y-1">
                ...
                <select name="material_id" required class="w-full py-2.5 px-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none">
                    @foreach($materials as $m)
                        <option value="{{ $m->id }}" {{ old('material_id', $bom->material_id) == $m->id ? 'selected' : '' }}>
            ...
            <div class="space-y-1">
                ...
                <input type="number" step="0.0001" name="qty_per_unit" value="{{ old('qty_per_unit', $bom->qty_per_unit) }}" required
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="unit_of_work" value="{{ old('unit_of_work', $bom->unit_of_work) }}" required
                       class="w-full py-2.5 px-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none">
            </div>
            ...
            <div class="space-y-1">
                ...
                <input type="number" step="0.1" name="allowable_variance_pct" value="{{ old('allowable_variance_pct', $bom->allowable_variance_pct) }}" min="0" max="100" required
                       class="w-full py-2.5 px-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none">
            </div>

            <!-- Project Scope -->
            <div class="space-y-1">
                <label class="block text-xs font-bold uppercase text-slate-600 dark:text-slate-300">Project Scope</label>
                <select name="project_id" class="w-full py-2.5 px-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none">
                    ...
                    @foreach($projects as $p)
                        <option value="{{ $p->id }}" {{ old('project_id', $bom->project_id) == $p->id ? 'selected' : '' }}>
        ...
        <div class="pt-4 border-t border-gray-100 dark:border-slate-800 flex justify-between items-center">
            <button type="button" onclick="if(confirm('Are you sure you want to delete this BOM template?')) document.getElementById('delete-bom-form').submit();"
    ...
    <form id="delete-bom-form" method="POST" action="{{ route('inventory.bom.destroy', $bom) }}" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</div>
